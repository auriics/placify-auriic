...
export const Pipeline: React.FC = () => {
  ...
  const filteredCandidates = useMemo(() => {
    return candidates.filter(c => {
      ...
      } else if (user?.role === 'jpc_resume') {
        return String(c.assigned_resume) === String(user.id);
      }
      
      return true;
    });
  }, [candidates, search, user, entityFilter]);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          <select
            ...
            className="bg-bg-secondary border border-border-primary rounded-2xl px-4 py-3 text-sm text-text-primary focus:outline-none focus:border-accent-blue transition-colors shadow-sm"
          >
