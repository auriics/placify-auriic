...
export const Pipeline: React.FC = () => {
  ...
  const groupedCandidates = useMemo(() => {
    ...
    candidates.forEach(c => {
      ...
      if (user?.role === 'jpc_recruiter' && String(c.assigned_recruiter) !== String(user.id)) return;
      if (user?.role === 'jpc_lead_gen' && String(c.lead_generated_by) !== String(user.id)) return;
      if (user?.role === 'jpc_marketing' && String(c.assigned_marketing_leader) !== String(user.id) && String(c.assigned_recruiter) !== String(user.id)) return;
      if (user?.role === 'jpc_resume' && String(c.assigned_resume) !== String(user.id)) return;
    ...
    return groups;
  }, [candidates, debouncedSearch, user, entityFilter]);
