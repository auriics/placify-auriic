...
type TabType = 'today' | 'upcoming' | 'pending_bookings' | 'booked' | 'live' | 'completed' | 'cancelled' | 'rescheduled' | 'self_attended' | 'analytics';
      ...
      <div className="bg-bg-secondary p-2 rounded-[32px] border border-border-primary/50">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 flex overflow-x-auto no-scrollbar p-1">
            {(['analytics', 'today', 'upcoming', 'pending_bookings', 'booked', 'live', 'self_attended', 'completed', 'cancelled', 'rescheduled'] as TabType[]).map((tab) => {
...
const ProxyScheduleTimeline: React.FC<{
  ...
  // Hours from 9:30 to 18:30 (6:30 PM)
  const hours = [];
  for (let h = 9; h <= 18; h++) {
  ...
  const proxyRounds = rounds.filter(r => 
    String(r.proxy_user_id) === proxyId && 
    (r.booked_slot_time || r.interview_date || '').startsWith(targetDate) &&
    r.status !== 'cancelled'
  );

  const proxySlots = availabilities.filter(a =>
    String(a.proxy_user_id) === proxyId &&
    a.slot_start.startsWith(targetDate)
  );
  ...
  return (
    <motion.div 
      initial={{ opacity: 0, y: -20 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-bg-secondary p-8 rounded-[40px] border border-border-primary shadow-sm mb-8"
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-black text-text-primary tracking-tight">Schedule for {proxy.display_name}</h3>
            <p className="text-xs font-bold text-text-muted uppercase tracking-[0.2em]">{new Date(targetDate).toLocaleDateString('en-US', { dateStyle: 'full' })}</p>
          </div>
        </div>
        <div className="flex items-center gap-4 bg-bg-tertiary px-6 py-3 rounded-2xl border border-border-primary">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-accent-blue" />
            <span className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Interview</span>
          </div>
          <div className="w-px h-4 bg-border-primary" />
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-bg-secondary border border-border-primary" />
            <span className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Available</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-9 gap-3">
        {hours.map(({ h, m }) => {
          ...
          const round = proxyRounds.find(r => {
             const start = r.booked_slot_time || r.interview_date || '';
             return start.includes(timeStr);
          });

          const isAvailable = proxySlots.some(s => s.slot_start.includes(timeStr) && s.slot_status === 'available');
          const isBooked = proxySlots.some(s => s.slot_start.includes(timeStr) && s.slot_status === 'booked');

          return (
            <div 
              key={timeStr}
              className={cn(
                "p-4 rounded-2xl border transition-all flex flex-col justify-between h-28 group hover:-translate-y-1",
                round || isBooked ? "bg-accent-blue/10 border-accent-blue/30 shadow-lg shadow-accent-blue/5" : 
                isAvailable ? "bg-bg-secondary border-accent-blue/20" : 
                "bg-bg-tertiary/30 border-border-primary/50 opacity-40 grayscale"
              )}
            >
              <div className="flex justify-between items-start">
                ...
                {round && <span className="px-1.5 py-0.5 rounded-md bg-accent-blue text-white text-[8px] font-black uppercase">Live</span>}
              </div>
              
              <div className="mt-auto pt-2 overflow-hidden">
                {round ? (
                  <p className="text-[9px] font-black text-accent-blue leading-tight line-clamp-2">
                    {(() => {
                      const req = requests.find(r => r.id === round.request_id);
                      const cand = candidates.find(c => c.id === req?.candidate_id);
                      return cand?.full_name || 'Interview';
                    })()}
                  </p>
                ) : isAvailable ? (
                  <p className="text-[9px] font-bold text-text-muted uppercase tracking-widest opacity-60">Free</p>
                ) : (
                  <p className="text-[9px] font-bold text-text-muted/40 uppercase tracking-widest">N/A</p>
                )}
              </div>
            </div>
          );
