...
export async function checkProxyAvailability(
  ...
  try {
    ...
    const body = {
      timeMin: startTime, // ISO string
      timeMax: endTime,   // ISO string
      items: [{ id: calendarId }]
    };
    ...
    if (!response.ok) {
      ...
      console.error('[CalendarService] FreeBusy API Error:', errorText);
      return { isAvailable: false, error: 'Failed to check Google Calendar availability.' };
    }

    const data = await response.json();
    const busy = data.calendars[calendarId]?.busy || [];
    
    return { isAvailable: busy.length === 0 };
  } catch (error) {
